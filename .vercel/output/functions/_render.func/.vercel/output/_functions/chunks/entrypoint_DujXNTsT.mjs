...
import {
	...
	K as removeTrailingForwardSlash,
	L as RewriteWithBodyUsed,
	Q as LocalsNotAnObject,
	S as PrerenderClientAddressNotAvailable,
	T as ClientAddressNotAvailable,
	U as StaticClientAddressNotAvailable,
	V as AstroResponseHeadersReassigned,
	W as fileExtension,
	X as slash,
	Y as prependForwardSlash,
} from "./astro/assets-service_DgXN-x9d.mjs";
import {
...
} from "./astro-designed-error-pages_BQxRyLnN.mjs";
import "fast-glob";
...
class RenderContext {
	...
	computeCurrentLocale() {
		...
		let computedLocale;
		const pathname =
			routeData.pathname && !isRoute404or500(routeData)
				? routeData.pathname
				: url.pathname;
...
class NodeApp extends App {
	...
	static createRequest(req, { skipBody = false } = {}) {
		...
		const hostname =
			forwardedHostname ?? req.headers.host ?? req.headers[":authority"];
		const port = getFirstForwardedValue(req.headers["x-forwarded-port"]);
		const portInHostname =
			typeof hostname === "string" && /:\d+$/.test(hostname);
